...
function timeFrameCalculation(durations){
    ...
    let timeFrames = [];
    passingTime = 5
    startTime = new Date(2023,9,5, 7,40);
    ...
    //print
     ...
     if(startTime.getHours()>12)
       temp = (startTime.getHours()-12) + ":";

    else
      temp = (startTime.getHours()) + ":";

    if(startTime.getMinutes()<10)
        timeFrames.push(temp + "0" + startTime.getMinutes())

    else
        timeFrames.push(temp + startTime.getMinutes())
    ...
    //adds the length of the period to the start time of the period 
     ...
     if(startTime.getHours()>12)
        timeFrames[i] = timeFrames[i] + "-" + (startTime.getHours()-12) + ":" ;  

    else
         timeFrames[i] = timeFrames[i] + "-" + startTime.getHours() + ":";

     if(startTime.getMinutes()<10)
         timeFrames[i] = timeFrames[i] +"0" + startTime.getMinutes();
         
    else
        timeFrames[i] = timeFrames[i] + startTime.getMinutes();
    ...
    
    {
        ...
        while (timeCounter <= timeTaken) {
            ...
            for (let i = 1; (i < 9 ) ; i++) {
                ...
                if (i != 4 && changed_periods[i] == false) {
		...
		
        {
			...
			
            {
                ...
                
                {
                    ...
                    if (i != 5 && changed_periods[i] == false) {
    ...
    if (newPeriodThree > durations[6]) {
        ...
        while(timeCounter<=timeTaken) { 
            for(let i = 1; (i<9);i++) {
                ...
                if (i != 6 && changed_periods[i] == false) {
		...
		if (newPeriodEleven > durations[7])
		{
			...
			while (timeCounter<=timeTaken)
			{
				...
				for(let i = 1;(  (i<9));i++) {
                    ...
                    if(i!=7 && changed_periods[i] == false){}
		...
		if(newPeriodTwelve > durations[8]) {
			...
			while(timeCounter<=timeTaken) {
                ...
                
                {
                    ...
                    if (i != 8 && changed_periods[i] == false) {
		 ...
		 if (newPeriodTwelve < durations[8]) {
				...
				while (timeCounter2<=timeGiven) {
					...
					for (let i = 1; (i<9) ; i++)
					{
                        ...
                        if (i != 9 && changed_periods[i] == false) {
...
function calculate() {
    ...
    for (let i = 0; i < listOfEditedPeriods[0].length; i++) { 
        ...
        if (listOfEditedPeriods[0][i] == 1) {
            ...
            mainScheduleDurations = periodOne(listOfEditedPeriods[1][i],mainScheduleDurations, 
                changed_periods)
...
booo = true;
    changed_periods = [false, false, false, false, false, false, false, false, false];
    mainScheduleDurations = [5, 13, 13, 13, 42, 42, 42, 42, 42, 24, 24, 24, 24];
